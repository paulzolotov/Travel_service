...
def replace_text_with_censored(instance):
    ...
    instance = list(serializers.deserialize("json", instance))[
        0
    ].object  # из json строки будем преобразовывать в
...
def weekly_notification_task():
    ...
    """
    all_users = list(
        User.objects.filter(is_staff=False).values()
    )  # выбираем пользователей, которые не имеют
